  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(report => 
        ...
        (report.reporter && report.reporter.name && report.reporter.name.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
      ...
      <Card>
        <Card.Body>
          {filteredReports.length === 0 ? (
            <p className="text-center">No reports found matching your criteria.</p>
          ) : (
                ...
                <tbody>
                  {filteredReports.map(report => (
                    <tr key={report._id}>
                      ...
                      <td>{report.assignedTo ? report.assignedTo.name : 'Unassigned'}</td>
